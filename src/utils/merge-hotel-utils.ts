import { Amenities, Hotel, Image, ImageCategory } from "../models/hotel-interfaces";
...
export class MergeUtil {
  ...
  mergeHotels() {
    MergeUtil.hotels.forEach((hotel) => {
      ...
      if (!MergeUtil.hotelMap[id]) {
        ...
        // Merge Names
        MergeUtil.hotelMap[id].name = MergeUtil.mergeNames(currentHotel, mergingHotel);
        // Merge Locations
        MergeUtil.hotelMap[id].location = MergeUtil.mergeLocations(currentHotel,mergingHotel);
        // Merge Descriptions
        MergeUtil.hotelMap[id].description = MergeUtil.mergeDescriptions(currentHotel, mergingHotel);
        // Merge amenities
        MergeUtil.hotelMap[id].amenities = MergeUtil.mergeAmenities(currentHotel,mergingHotel);
        // Merge Booking Conditions
        MergeUtil.hotelMap[id].booking_conditions = MergeUtil.mergeBookingConditions(currentHotel, mergingHotel);
        // Merge Images
        MergeUtil.hotelMap[id].images = MergeUtil.mergeImages(currentHotel, mergingHotel);
      }
    });
  ...
  static mergeImages(currentHotel: Hotel, mergingHotel: Hotel): ImageCategory {
    ...
    const mergeAndRemoveDuplicates = (currentImages: Image[], mergingImages: Image[]): Image[] => {
    ...
    image.rooms = mergeAndRemoveDuplicates(currentHotel.images.rooms, mergingHotel.images.rooms);
    image.site = mergeAndRemoveDuplicates(currentHotel.images.site, mergingHotel.images.site);
    image.amenities = mergeAndRemoveDuplicates(currentHotel.images.amenities, mergingHotel.images.amenities);
    return image;
...
// Booking conditions
static mergeBookingConditions(currentHotel: Hotel, mergingHotel: Hotel): string[] {
    return MergeUtil.createUniqueSet(currentHotel.booking_conditions, mergingHotel.booking_conditions);

}
//   Longer name priority
  static mergeNames(currentHotel: Hotel, mergingHotel: Hotel): string {
    return currentHotel.name.length > mergingHotel.name.length ? currentHotel.name : mergingHotel.name;
  }
...
static mergeDescriptions(currentHotel: Hotel, mergingHotel: Hotel): string {
    ...
    return currentDescription.length > mergingDescription.length ? currentDescription : mergingDescription;
}
