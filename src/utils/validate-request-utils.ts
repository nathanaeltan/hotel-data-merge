...
export function validateHotelRequest(hotelRequest: HotelRequest): { error: string } | null {
    if (hotelRequest.hotel_ids) {
      if (!Array.isArray(hotelRequest.hotel_ids) || !hotelRequest.hotel_ids.every(id => typeof id === 'string')) {
